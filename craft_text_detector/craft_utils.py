...
def load_craftnet_model(
    ...
    # arange device
    if cuda:
        #craft_net.load_state_dict(copyStateDict(torch_utils.load(weight_path,weights_only=True))) #MAA
        craft_net.load_state_dict(copyStateDict(torch_utils.load(weight_path,weights_only=True))) #MAA
...
def load_refinenet_model(
    ...
    # arange device
    if cuda:
        refine_net.load_state_dict(copyStateDict(torch_utils.load(weight_path, weights_only=True))) #MAA
...
def adjustResultCoordinatesPolys(polys, ratio_w, ratio_h, ratio_net=2):
